...
export class ScenarioPackService {
  ...
  private observations(id:AgentAuthorityScenarioId,receipts:ReturnType<ScenarioPackService["readReceipts"]>){if(id!=="AA-004")return [];const summaries:Record<string,string>={inspect_user_profile:"BACKUPADM is an old enabled privileged service profile.",inspect_object_authority:"BACKUPADM currently has *ALL private authority on PAYROLL/PAYMST.",list_recent_audit_events:"Recent BACKUPADM audit activity was reviewed within a bounded result set."};return receipts.filter((r)=>r.type==="read_allowed").map((r)=>{const action=r.payload.action as Record<string,unknown>|undefined;const tool=String(action?.tool_name??action?.toolName??"observation");return {receiptId:r.id,tool,summary:summaries[tool]??"Observation recorded.",approvalRequired:false};});}
  private status(id:AgentAuthorityScenarioId){const proposals=this.proposals(id);const receipts=this.readReceipts(id);if(id==="AA-005"&&receipts.some((r)=>r.type==="request_denied"))return "complete";if(id==="AA-004"&&!proposals.length&&receipts.length)return "in_progress";if(!proposals.length)return "not_started";if(proposals.some((p)=>p.status==="pending"))return "in_progress";if(id==="AA-002"&&proposals.some((p)=>JSON.parse(p.argumentsJson).authority==="*ALL"&&p.status==="consumed"))return "complete_with_warning";return "complete";}

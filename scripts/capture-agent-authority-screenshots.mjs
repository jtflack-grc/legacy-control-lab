...
try{
  ...
  await page.goto("http://127.0.0.1:8080/lab/?path=agentauthority");await wait(async()=>(await page.locator("#aa-status").textContent())?.includes("proof verified"));await page.locator("#aa-step-action").click();await wait(async()=>(await page.locator("#aa-step-count").textContent())==="Step 6 of 6");await page.locator("#aa-evidence-card").scrollIntoViewIfNeeded();await page.waitForTimeout(1000);await page.screenshot({path:`${root}/public/assets/screenshots/05-agent-authority-complete.png`,fullPage:true});closeDatabase();
  await browser.close();

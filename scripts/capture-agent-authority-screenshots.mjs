...
try{
  ...
  await page.locator("#aa-start-request").click();await wait(async()=>(await page.locator("#aa-status").textContent())?.includes("Awaiting decision"));await page.waitForTimeout(1000);await page.screenshot({path:`${root}/public/assets/screenshots/04-agent-authority-pending.png`,fullPage:true});
  ...
  await page.goto("http://127.0.0.1:8080/lab/?path=agentauthority");await wait(async()=>(await page.locator("#aa-status").textContent())?.includes("proof verified"));await page.locator("#aa-evidence-card").scrollIntoViewIfNeeded();await page.waitForTimeout(1000);await page.screenshot({path:`${root}/public/assets/screenshots/05-agent-authority-complete.png`,fullPage:true});closeDatabase();
  await browser.close();

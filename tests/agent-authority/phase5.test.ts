...
describe("Agent Authority Phase 5 Authority Desk",()=>{
  ...
  it("serves a framework-free Desk with exact operator discovery and safe DOM construction",async()=>{
    ...
    expect(html).toContain("Authority Desk");expect(html).toContain("Recent decisions");expect(html).toContain("Back to Agent Authority Lab");expect(html).toContain("fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;600&amp;family=IBM+Plex+Sans:wght@300;400;500;600&amp;display=swap");expect(js).toContain("user=QSECOFR");expect(js).not.toContain("Sec-Fetch-Site");expect(js).toContain("textContent");expect(js).toContain("replaceChildren");expect(js).not.toMatch(/innerHTML|insertAdjacentHTML|\beval\s*\(/);expect(js).toContain("action_hash:proposal.actionHash");expect(js).toContain("await refresh()");expect(js).not.toMatch(/approvedBy|approverUser|counterfactual/);expect(css).toContain('--sans:"IBM Plex Sans",-apple-system,BlinkMacSystemFont,"SF Pro Text","Helvetica Neue",Arial,sans-serif');expect(css).toContain('--mono:"IBM Plex Mono","Cascadia Mono",Consolas,ui-monospace,monospace');expect(css).toContain("rgba(127,255,178,.018)");
  });

  it("keeps locked and verified operator states legible in the portfolio-derived Desk",async()=>{
    const enabled=await mountEnabled();const executable=browserPath();const browser=await chromium.launch({headless:true,...(executable?{executablePath:executable}:{}),args:["--no-sandbox"]});cleanup.push(()=>browser.close());const page=await browser.newPage();await page.route(/https:\/\/fonts\.(googleapis|gstatic)\.com\/.*/,route=>route.abort());await page.goto(`${enabled.url}/lab/authority/`);await expect.poll(()=>page.locator("#locked").isVisible()).toBe(true);expect(await page.locator("#locked").textContent()).toContain("Authority Desk locked");expect(await page.locator("#readiness").textContent()).toContain("Operator session required");expect(await page.locator("body").evaluate((node)=>getComputedStyle(node).fontFamily)).toContain("IBM Plex Sans");liveSession("QSECOFR");await page.getByRole("button",{name:"Refresh"}).click();await expect.poll(()=>page.locator("#readiness").textContent()).toContain("Operator session verified");expect(await page.locator("#readiness").textContent()).toContain("QSECOFR");
  },30_000);

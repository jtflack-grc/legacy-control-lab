...
describe("Agent Authority Phase 6.5 guided LCL experience",()=>{
  ...
  it("guides AA-001 from untrusted message to exact approval, shared state, evidence and proof",async()=>{
    ...
    const operator=liveOperator();await expect.poll(()=>page.locator("#aa-step-count").textContent()).toBe("Step 4 of 6");expect(await page.getByRole("button",{name:"Review request in Authority Desk"}).isVisible()).toBe(true);
